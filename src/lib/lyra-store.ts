import type { Locale } from "./lyra-i18n";
...
export type LyraSettings = { theme: Theme; locale: Locale; performanceMode: boolean; historyEnabled: boolean; privacyAnalytics: boolean; voice: "female-natural"|"male-natural"|"female-soft"|"male-expressive" };
const CONVERSATIONS_KEY="lyra-ai:conversations"; const SETTINGS_KEY="lyra-ai:settings";
export const DEFAULT_SETTINGS: LyraSettings={ theme:"dark", locale:"pt", performanceMode:false, historyEnabled:true, privacyAnalytics:false, voice:"female-natural" };
...
export function loadSettings():LyraSettings{if(typeof window==="undefined")return DEFAULT_SETTINGS;try{const p=JSON.parse(localStorage.getItem(SETTINGS_KEY)??"{}");return{...DEFAULT_SETTINGS,...p,theme:["dark","light","system"].includes(p.theme)?p.theme:"dark",locale:["pt","en","es","fr","de"].includes(p.locale)?p.locale:"pt"};}catch{return DEFAULT_SETTINGS;}}
export function saveSettings(s:LyraSettings){if(typeof window!=="undefined")localStorage.setItem(SETTINGS_KEY,JSON.stringify(s));}
