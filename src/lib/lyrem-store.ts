...
export const DEFAULT_SETTINGS: LyremSettings={ theme:"light", locale:"pt", performanceMode:false, historyEnabled:true, privacyAnalytics:false, voice:"female-natural" };
...
export function loadSettings():LyremSettings{if(typeof window==="undefined")return DEFAULT_SETTINGS;try{const p=JSON.parse(localStorage.getItem(SETTINGS_KEY)??"{}");return{...DEFAULT_SETTINGS,...p,theme:["dark","light","system"].includes(p.theme)?p.theme:"light",locale:["pt","en","es","fr","de"].includes(p.locale)?p.locale:"pt"};}catch{return DEFAULT_SETTINGS;}}
