...
import { randomSeed } from "./lyra-image";
...
export const LYRA_VIDEO_SCHEME = "lyra-video:";
...
export function videoMarkdownSrc(prompt: string) {
  return `${LYRA_VIDEO_SCHEME}${encodeURIComponent(prompt)}`;
}

export function isVideoSrc(src: string) {
  return src.startsWith(LYRA_VIDEO_SCHEME);
}

export function promptFromVideoSrc(src: string) {
  try {
    return decodeURIComponent(src.slice(LYRA_VIDEO_SCHEME.length));
  } catch {
    return src.slice(LYRA_VIDEO_SCHEME.length);
  }
}
